...
class EmployeeTaggingForm(forms.ModelForm):
    ...
    fromDate = forms.DateField(label='From Date', widget=forms.TextInput(attrs={'class': 'form-control ', 'type': 'date', 'id': 'inputFromDate', 'placeholder': 'Select From-Date', 'autocomplete':'off',}),input_formats=['%Y-%d-%m'], required=True)
    toDate = forms.DateField(label='To Date', widget=forms.TextInput(attrs={'class': 'form-control ', 'type': 'date', 'id': 'inputToDate', 'placeholder': 'Select To-Date','autocomplete': 'off', 'disabled': True}),input_formats=['%Y-%d-%m'], required=True)
